...
def load_data():
    ...
    try:
        ...
        sys.stderr.write("There was an error in the SVN call, please ensure your credentials are correct")
        sys.exit(0)
...
def parse_committees_info():
    ...
    projects_processed, pmcs, roster_counts, committees, boards_processed = parse_rosters(COMMITTEES[3])
    ...
    for chair in chairs.split('\n')[2:-3]:
        ...
        if key in pmcs.keys():        
            pmcs[key]["chair"] = {} 
            pmcs[key]["chair"][c[1].split()[0]] = {"name": c[1].split()[1].split("@")[0]}
        else:
            continue
    officers_temp = {}
...
#        officers_temp[set_key(b[0])] = { "display_name": b[0],"paragraph": "Committee", "roster": { f"{temp_roster[2]}": { "name": temp_roster[0]}}}
    ...
    for executive in execs.split('\n')[2:-3]:
        ...
        temp_roster.append(temp_roster[1].split('@')[0])
        officers_temp[set_key(x[0])] = { "display_name": x[0],"paragraph": "Executive Officer", "roster": { f"{temp_roster[2]}": { "name": temp_roster[0]}}}
    ...
    for officer in officers.split('\n')[2:-9]:
        ...
        temp_roster.append(temp_roster[1].split('@')[0])
        officers_temp[set_key(o[0])] = { "display_name": o[0],"paragraph": "Additional Officers", "roster": { f"{temp_roster[2]}": { "name": temp_roster[0]}}}
